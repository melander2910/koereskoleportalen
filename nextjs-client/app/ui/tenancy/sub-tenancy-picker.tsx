...
export default function SubTenancyPicker({
  ...
  const {
    ...
    queryKey: ['userSubTenants'],
    queryFn: async () => {
      const response = await backOfficeClient.getAllProductionUnits();
  ...
  return (
    <div className="flex h-full flex-col px-3 py-4 md:px-2">
      ...
      <div className="flex grow flex-row justify-between space-x-2 md:flex-col md:space-x-0 md:space-y-2">
        {userSubTenantsData?.map((tenant: any) => {
          return (
            <Link
              key={tenant.cvr}
              href={`/${tenant.cvr}`}
              className={clsx(
                ...
                {
                  'bg-sky-100 text-blue-600': pathname === tenant.cvr,
                },
              )}
            >
